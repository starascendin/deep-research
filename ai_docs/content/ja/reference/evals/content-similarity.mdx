---
title: "リファレンス: Content Similarity | Evals | Mastra Docs"
description: 文字列間のテキスト類似度を測定し、マッチングスコアを提供するMastraのContent Similarity Metricのドキュメント。
---
...
`ContentSimilarityMetric`クラスは、2つの文字列間のテキストの類似性を測定し、それらがどの程度一致するかを示すスコアを提供します。大文字小文字の区別や空白の処理について設定可能なオプションをサポートしています。
