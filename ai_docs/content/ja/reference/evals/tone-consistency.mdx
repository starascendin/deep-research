---
title: "リファレンス: トーン一貫性 | メトリクス | Evals | Mastra Docs"
description: テキストの感情的なトーンと感情の一貫性を評価するMastraのトーン一貫性メトリクスのドキュメント。
---
...
`ToneConsistencyMetric`クラスは、テキストの感情的なトーンと感情の一貫性を評価します。このクラスは2つのモードで動作できます：入力/出力ペア間のトーンを比較するか、単一のテキスト内でのトーンの安定性を分析するかです。
