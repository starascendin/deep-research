...
description: 入力から重要なキーワードをLLM出力がどの程度カバーしているかを評価するMastraのキーワードカバレッジメトリクスのドキュメント。
---
...
`KeywordCoverageMetric`クラスは、LLMの出力が入力からの重要なキーワードをどの程度カバーしているかを評価します。一般的な単語やストップワードを無視しながら、キーワードの存在とマッチングを分析します。
