...
/**
 * Invoke the cloud-hosted basicAgent.
 *
 * Usage:
 *   npx tsx zscripts/run-basic-agent-cloud.ts "What's the weather in Tokyo in fahrenheit?"
...
async function main() {
  ...
  try {
    ...
    const resp = await agent.stream(messages as any, { maxSteps: 8 } as any);
